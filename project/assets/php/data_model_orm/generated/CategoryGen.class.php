 ...
 * @property integer $TotalExpenses the value for intTotalExpenses 
 * @property-read string $LastUpdated the value for strLastUpdated (Read-Only Timestamp)
 * @property integer $ObjectOwner the value for intObjectOwner 
 * @property-read Expenses $_Expenses the value for the private _objExpenses (Read-Only) if set due to an expansion on the Expenses.Category reverse relationship
 * @property-read Expenses[] $_ExpensesArray the value for the private _objExpensesArray (Read-Only) if set due to an ExpandAsArray on the Expenses.Category reverse relationship
...
class CategoryGen extends dxBaseClass implements IteratorAggregate {
    ...
    /**
     * Protected member variable that maps to the database column Category.TotalExpenses
     * @var integer intTotalExpenses
     */
    protected $intTotalExpenses;
    const TotalExpensesDefault = null;
    ...
    /**
     * Private member variable that stores a reference to a single Expenses object
     * (of type Expenses), if this Category object was restored with
     * an expansion on the Expenses association table.
     * @var Expenses _objExpenses;
     */
    private $_objExpenses;

    /**
     * Private member variable that stores a reference to an array of Expenses objects
     * (of type Expenses[]), if this Category object was restored with
     * an ExpandAsArray on the Expenses association table.
     * @var Expenses[] _objExpensesArray;
     */
    private $_objExpensesArray = null;
    ...
    public function Initialize() {
        ...
        $this->intTotalExpenses = Category::TotalExpensesDefault;
    ...
    public static function GetSelectFields(dxQueryBuilder $objBuilder, $strPrefix = null, dxQuerySelect $objSelect = null) {
        ...
        if ($objSelect) {
            ...
            $objBuilder->AddSelectItem($strTableName, 'TotalExpenses', $strAliasPrefix . 'TotalExpenses');
    ...
    */
    public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $objExpandAsArrayNode = null, $objPreviousItemArray = null, $strColumnAliasArray = array()) {
        ...
        $strAlias = $strAliasPrefix . 'TotalExpenses';
        $strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
        $objToReturn->intTotalExpenses = $objDbRow->GetColumn($strAliasName, 'Integer');
        ...
        // Check for Expenses Virtual Binding
        $strAlias = $strAliasPrefix . 'expenses__Id';
        $strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
        $objExpansionNode = (empty($objExpansionAliasArray['expenses']) ? null : $objExpansionAliasArray['expenses']);
        $blnExpanded = ($objExpansionNode && $objExpansionNode->ExpandAsArray);
        if ($blnExpanded && null === $objToReturn->_objExpensesArray)
            $objToReturn->_objExpensesArray = array();
        if (!is_null($objDbRow->GetColumn($strAliasName))) {
            if ($blnExpanded) {
                $objToReturn->_objExpensesArray[] = Expenses::InstantiateDbRow($objDbRow, $strAliasPrefix . 'expenses__', $objExpansionNode, null, $strColumnAliasArray);
            } elseif (is_null($objToReturn->_objExpenses)) {
                $objToReturn->_objExpenses = Expenses::InstantiateDbRow($objDbRow, $strAliasPrefix . 'expenses__', $objExpansionNode, null, $strColumnAliasArray);
            }
        }
    ...
    */
    public function Save($blnForceInsert = false, $blnForceUpdate = false) {
        ...
        if (!$ExistingObj) {
            ...
            $ChangedArray = array_merge($ChangedArray,array("TotalExpenses" => $this->intTotalExpenses));
            ...
            $ExistingValueStr = "NULL";
            if (!is_null($ExistingObj->TotalExpenses)) {
                $ExistingValueStr = $ExistingObj->TotalExpenses;
            }
            if ($ExistingObj->TotalExpenses != $this->intTotalExpenses) {
                $ChangedArray = array_merge($ChangedArray,array("TotalExpenses" => array("Before" => $ExistingValueStr,"After" => $this->intTotalExpenses)));
                //$ChangedArray = array_merge($ChangedArray,array("TotalExpenses" => "From: ".$ExistingValueStr." to: ".$this->intTotalExpenses));
            }
            $ExistingValueStr = "NULL";
        ...
        try {
            if ((!$this->__blnRestored) || ($blnForceInsert)) {
                ...
                INSERT INTO `Category` (
							...
							' . $objDatabase->SqlVariable($this->intTotalExpenses) . ',
            ...
            UPDATE `Category` SET
							...
							`TotalExpenses` = ' . $objDatabase->SqlVariable($this->intTotalExpenses) . ',
    ...
    public function Reload() {
        ...
        $this->intTotalExpenses = $objReloaded->intTotalExpenses;
    ...
    public function __get($strName) {
        switch ($strName) {
            ...
            case 'TotalExpenses':
                /**
                 * Gets the value for intTotalExpenses 
                 * @return integer
                 */
                return $this->intTotalExpenses;
            ...
            case '_Expenses':
                /**
                 * Gets the value for the private _objExpenses (Read-Only)
                 * if set due to an expansion on the Expenses.Category reverse relationship
                 * @return Expenses
                 */
                return $this->_objExpenses;

            case '_ExpensesArray':
                /**
                 * Gets the value for the private _objExpensesArray (Read-Only)
                 * if set due to an ExpandAsArray on the Expenses.Category reverse relationship
                 * @return Expenses[]
                 */
                return $this->_objExpensesArray;
    ...
    public function __set($strName, $mixValue) {
        switch ($strName) {
            ...
            case 'TotalExpenses':
                /**
                 * Sets the value for intTotalExpenses 
                 * @param integer $mixValue
                 * @return integer
                 */
                try {
                    return ($this->intTotalExpenses = dxType::Cast($mixValue, dxType::Integer));
                } catch (dxCallerException $objExc) {
                    $objExc->IncrementOffset();
                    throw $objExc;
                }
    ...
    // Related Objects' Methods for Expenses
    //-------------------------------------------------------------------

    /**
     * Gets all associated Expenseses as an array of Expenses objects
     * @param dxQueryClause[] $objOptionalClauses additional optional dxQueryClause objects for this query
     * @return Expenses[]
    */
    public function GetExpensesArray($objOptionalClauses = null) {
        if ((is_null($this->intId)))
            return array();

        try {
            return Expenses::LoadArrayByCategory($this->intId, $objOptionalClauses);
        } catch (dxCallerException $objExc) {
            $objExc->IncrementOffset();
            throw $objExc;
        }
    ...
    /**
     * Counts all associated Expenseses
     * @return int
    */
    public function CountExpenseses() {
        if ((is_null($this->intId)))
            return 0;

        return Expenses::CountByCategory($this->intId);
    }

    /**
     * Associates a Expenses
     * @param Expenses $objExpenses
     * @return void
    */
    public function AssociateExpenses(Expenses $objExpenses) {
        if ((is_null($this->intId)))
            throw new dxUndefinedPrimaryKeyException('Unable to call AssociateExpenses on this unsaved Category.');
        if ((is_null($objExpenses->Id)))
            throw new dxUndefinedPrimaryKeyException('Unable to call AssociateExpenses on this Category with an unsaved Expenses.');
        ...
        $objDatabase->NonQuery('
            ...
            WHERE
                `Id` = ' . $objDatabase->SqlVariable($objExpenses->Id) . '
        ');
    }

    /**
     * Unassociates a Expenses
     * @param Expenses $objExpenses
     * @return void
    */
    public function UnassociateExpenses(Expenses $objExpenses) {
        if ((is_null($this->intId)))
            throw new dxUndefinedPrimaryKeyException('Unable to call UnassociateExpenses on this unsaved Category.');
        if ((is_null($objExpenses->Id)))
            throw new dxUndefinedPrimaryKeyException('Unable to call UnassociateExpenses on this Category with an unsaved Expenses.');
        ...
        $objDatabase->NonQuery('
            ...
            SET
                `Category` = null
            WHERE
                `Id` = ' . $objDatabase->SqlVariable($objExpenses->Id) . ' AND
                `Category` = ' . $objDatabase->SqlVariable($this->intId) . '
        ');
    }

    /**
     * Unassociates all Expenseses
     * @return void
    */
    public function UnassociateAllExpenseses() {
        if ((is_null($this->intId)))
            throw new dxUndefinedPrimaryKeyException('Unable to call UnassociateExpenses on this unsaved Category.');
    ...
    /**
     * Deletes an associated Expenses
     * @param Expenses $objExpenses
     * @return void
    */
    public function DeleteAssociatedExpenses(Expenses $objExpenses) {
        if ((is_null($this->intId)))
            throw new dxUndefinedPrimaryKeyException('Unable to call UnassociateExpenses on this unsaved Category.');
        if ((is_null($objExpenses->Id)))
            throw new dxUndefinedPrimaryKeyException('Unable to call UnassociateExpenses on this Category with an unsaved Expenses.');
    ...
    /**
     * Deletes all associated Expenseses
     * @return void
    */
    public function DeleteAllExpenseses() {
        if ((is_null($this->intId)))
            throw new dxUndefinedPrimaryKeyException('Unable to call UnassociateExpenses on this unsaved Category.');
...
///////////////////////////////
    ...
    public static function GetSoapComplexTypeXml() {
        ...
        $strToReturn .= '<element name="TotalExpenses" type="xsd:int"/>';
    ...
    public static function GetObjectFromSoapObject($objSoapObject) {
        ...
        if (property_exists($objSoapObject, 'TotalExpenses'))
            $objToReturn->intTotalExpenses = $objSoapObject->TotalExpenses;
    ...
    public function getIterator() {
        ...
        $iArray['TotalExpenses'] = $this->intTotalExpenses;
...
/////////////////////////////////////
     ...
     * @property-read dxQueryNode $TotalExpenses
     * @property-read dxQueryNode $LastUpdated
     * @property-read dxQueryNode $ObjectOwner
     *
     *
     * @property-read dxQueryReverseReferenceNodeExpenses $Expenses
	...
	class dxQueryNodeCategory extends dxQueryNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'TotalExpenses':
					return new dxQueryNode('TotalExpenses', 'TotalExpenses', 'Integer', $this);
				case 'LastUpdated':
					return new dxQueryNode('LastUpdated', 'LastUpdated', 'VarChar', $this);
				case 'ObjectOwner':
					return new dxQueryNode('ObjectOwner', 'ObjectOwner', 'Integer', $this);
				case 'Expenses':
					return new dxQueryReverseReferenceNodeExpenses($this, 'expenses', 'reverse_reference', 'Category', 'Expenses');
